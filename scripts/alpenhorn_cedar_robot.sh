...
# alpenhorn start-up script for robot node at cedar.
...
#
# ssh robot.cedar.alliancecan.ca -i <chimedat-robot-keyfile> \
#    { start | stop | restart }
#
#
# Available commands:
...
then
  ...
  then
    ...
    run_screen -d -m -S alpenhornd /usr/bin/sg rpp-chime ${HOME}/bin/alpenstart
  else
